...
)
{
    ...
    if is_runtime_save_target(&target)
    {
        return;
    }
    let category = categorize_content_target(ContentCategoryTarget { kind: &context.root.kind, target_lower: &target });
    let entry = by_target
        .entry(target)
...
fn is_runtime_save_target(target: &str) -> bool
{
    return target
        .split('/')
...
fn categorize_content_target(target: ContentCategoryTarget<'_>) -> ContentCategory
{
    ...
    if has_segment_needle(SegmentNeedle { lower: &lower, needle: "modloader" })
    {
