...
fn ensure_journal_allowed(game_root: &Path, journal_path: &Path) -> Result<(), AppError> {
    ...
    let journal = journal_path.canonicalize()?;
...
fn parse_rollback_journal(content: &str) -> RollbackJournal {
...
fn parse_rollback_line(line: &str, journal: &mut RollbackJournal) {
    if let Some(value) = line.strip_prefix("backup=") {
        ...
        if let Some(dest) = fields.first() {
            ...
            journal.new_files.push(NewFileEntry {
                dest: PathBuf::from(unescape_value(dest)),
